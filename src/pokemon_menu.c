...
void PokemonMenu_FieldMove(u8 taskId)
{
    ...
    else if(data[11] <= 7 && FlagGet(data[11] + BADGE01_GET) != TRUE && (data[11] != 5 || CheckSpeedchoiceOption(EARLYFLY, YES) == FALSE) && (data[11] != 6 || (CheckSpeedchoiceOption(AQUALESS, KEEP) == TRUE))) // EARLY FLY
    {
        MenuZeroFillWindowRect(0x13, 0, 0x1D, 0x13);
        sub_806D5A4();
