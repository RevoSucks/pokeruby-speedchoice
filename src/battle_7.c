...
void sub_80313A0(struct Sprite *sprite)
{
    ...
    {
		sprite->pos2.x += sprite->data0 * 1.5;
        if (sprite->pos2.x == 0)
            sprite->callback = SpriteCallbackDummy;
    }
}
...
void sub_80328A4(struct Sprite *sprite)
{
    ...
    if (ewram17800[r4].unk0_2)
        invisible = TRUE;
    sprite->pos1.x = (r7->pos1.x); // because the mon sprites are being moved twice as fast, do not multiply the sprite to adjust for that.
...
{
    gBattleBankFunc[gActiveBank] = sub_8032AFC;
}
...
void sub_8032AFC(void)
{
    if (gBattleExecBuffer & gBitTable[gActiveBank])
    {
        if (BattleBufferA[gActiveBank][0] <= 0x38)
            gUnknown_081FAF5C[BattleBufferA[gActiveBank][0]]();
        else
            sub_80334EC();
    }
}
